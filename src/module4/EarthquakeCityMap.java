...
public class EarthquakeCityMap extends PApplet {
	...
	// We will use member 	, instead of local variables, to store the data
	...
	private void addKey() {	
		...
		text("  City Marker",60,105);
		text("  Land Quake",60,155);
		text("  Ocean Quake",60,205);
		text("Size ~ Magnitude",50,225);
		text("  Shallow",60,250);
		text("  Intermediate",60,270);
		text("  Deep",60,290);
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker m : countryMarkers) {
			boolean temp= isInCountry(earthquake,m);
			if(temp) {
				return temp;
			}
	...
	{		int ocean_counter=0;
			for(Marker cm: countryMarkers) {
				...
				String name = (String)cm.getProperty("name");
					...
					if(quake_counter>0) {System.out.println(name+": "+quake_counter);}
					
				
			}
			if(ocean_counter>0) {System.out.println("OCEAN QUAKES"+": "+ocean_counter);}
